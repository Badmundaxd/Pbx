...
async def new_sticker_set(
    ...
    # ── FIX: only pass animated/videos flags when True ──
    # CreateStickerSet in newer TL schema throws:
    #   "unexpected keyword argument 'animated'"
    # when animated=False is passed explicitly.
    # Solution: build kwargs dict and only include the flag that applies.
    kwargs = dict(
    ...
    # static packs: no extra flag needed at all

    return await client.invoke(CreateStickerSet(**kwargs))
