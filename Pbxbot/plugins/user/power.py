...
@on_message("restart", allow_stan=True, Bad_user=True)
...
@on_message("shutdown", allow_stan=True, Bad_user=True)
async def shutdown_bot(_, message: Message):
    await Pbxbot.edit(
        message,
        "**[ ⚠️ ]** __Pbxbot 4.0 is now offline! Manually start again to get it back online.__",
    )
    try:
        if HEROKU_APP:
            try:
...
@on_message("cleanup", allow_stan=True, Bad_user=True)
async def clenup_bot(_, message: Message):
    await Pbxbot.edit(message, "**♻️ Pbxbot 4.0 Cleanup Completed!**")
    await restart(clean_up=True)


@on_message("update", allow_stan=True, Bad_user=True)
