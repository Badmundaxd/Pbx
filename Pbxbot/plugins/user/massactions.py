...
from . import HelpMenu, group_n_channel, group_only, Pbxbot, on_message, special


@on_message("banall", chat_type=group_n_channel, admin_only=True, allow_stan=True, Bad_user=True, enable_log=True)
@special
...
@on_message("unbanall", chat_type=group_n_channel, admin_only=True, allow_stan=True, Bad_user=True, enable_log=True)
...
@on_message("kickall", chat_type=group_n_channel, admin_only=True, allow_stan=True, Bad_user=True, enable_log=True)
@special
...
@on_message("muteall", chat_type=group_n_channel, admin_only=True, allow_stan=True, Bad_user=True, enable_log=True)
@special
async def muteall(client: Client, message: Message):
    ...
    mute_right = await message.chat.get_member(client.me.id)
    if not mute_right.privileges.can_restrict_members:
        return await Pbxbot.delete(
            message,
            f"__I don't have enough rights to mute users in {chat_name}.__\n\n__Give me permission to restrict users and try again.__",
        )

    Pbx = await Pbxbot.edit(message, f"__Muting all users in {chat_name}.__")
    ...
    await Pbxbot.check_and_log(
        "muteall",
        f"**Muteall In:** {chat_name} \n**Total:** {total} \n**Muted:** {success} \n**Failed:** {total - success}\n\n**By:** {client.me.mention}",
    )


@on_message("unmuteall", chat_type=group_n_channel, admin_only=True, allow_stan=True, Bad_user=True, enable_log=True)
async def unmuteall(client: Client, message: Message):
    ...
    if not mute_right.privileges.can_restrict_members:
        return await Pbxbot.delete(
            message,
            f"__I don't have enough rights to unmute users in {chat_name}.__\n\n__Give me permission to restrict users and try again.__",
        )

    Pbx = await Pbxbot.edit(message, f"__Unmuting all users in {chat_name}.__")
    ...
    await Pbxbot.delete(
        Pbx,
        f"Unmuteall Executed! \n\n__Total:__ {total} \n__Unmuted:__ {success} \n__Failed:__ {total - success}",
    )
    await Pbxbot.check_and_log(
        "unmuteall",
        f"**Unmuteall In:** {chat_name} \n**Total:** {total} \n**Unmuted:** {success} \n**Failed:** {total - success}\n\n**By:** {client.me.mention}",
    )
...
@on_message("blockall", chat_type=group_only, allow_stan=True, Bad_user=True)
async def blockall(client: Client, message: Message):
    ...
    Pbx = await Pbxbot.edit(message, f"__Blocking all users in {chat_name}.__")
...
@on_message("unblockall", chat_type=group_only, allow_stan=True, Bad_user=True)
...
@on_message("inviteall", allow_stan=True, Bad_user=True)
...
HelpMenu("massactions").add(
    ...
    "muteall",
    "<chatId>",
    "Mute all members in a group/channel. If no chatId is given, the command will be executed in the current chat.",
    "muteall -100xxxxxxxxx",
).add(
    "unmuteall",
    "<chatId>",
    "Unmute all members in a group/channel. If no chatId is given, the command will be executed in the current chat.",
    "unmuteall -100xxxxxxxxx",
).add(
    "deleteall",
    None,
