...
FONTS_PATH = "./Pbxbot/resources/fonts/font/*"
BLACK_BG   = "./Pbxbot/resources/fonts/blackbg.jpg"
CAPTION    = "Made by @PBXCHATS"
OUTPUT     = "LogoMake.png"


# ─────────────────────────────────────────────
#  HELPER — pick a random font
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
#  HELPER — draw centered text on image
# ─────────────────────────────────────────────
def _draw_text(img: Image.Image, text: str, font, fill, stroke_fill=None, stroke_width=0):
    draw = ImageDraw.Draw(img)
    W, H = img.size
    bbox = draw.textbbox((0, 0), text, font=font)
    w = bbox[2] - bbox[0]
    h = bbox[3] - bbox[1] + int((bbox[3] - bbox[1]) * 0.21)
    x = (W - w) / 2
    y = (H - h) / 2
    # base layer
    draw.text((x, y), text, font=font, fill=fill)
    # stroke layer (offset by 6px)
    if stroke_fill:
        draw.text((x, y + 6), text, font=font, fill="black",
                  stroke_width=stroke_width, stroke_fill=stroke_fill)


# ─────────────────────────────────────────────
#  HELPER — send photo and cleanup
# ─────────────────────────────────────────────
async def _send_logo(message: Message, status_msg, fname: str):
    await message.reply_photo(photo=fname, caption=CAPTION)
    os.remove(fname)
    await status_msg.delete()


# ─────────────────────────────────────────────
#  .logo — random background + white text
# ─────────────────────────────────────────────
@on_message("logo", allow_stan=True, Bad_user=True)
async def logo(client: Client, message: Message):
    text = message.text.split(' ', 1)[1] if ' ' in message.text else None
    if not text:
        return await message.reply_text("Please provide text after `.logo`")

    msg = await message.reply_text("⏳ Processing...")
    try:
        response = requests.get(random.choice(LOGO_LINKS), timeout=10)
        response.raise_for_status()
        img = Image.open(io.BytesIO(response.content))
        img.verify()
        img = Image.open(io.BytesIO(response.content))

        font = _random_font(120)
        _draw_text(img, text, font, fill=(255, 255, 255), stroke_fill="black", stroke_width=1)
        img.save(OUTPUT, "png")
        await _send_logo(message, msg, OUTPUT)
    except Exception as e:
        await msg.delete()
        await message.reply_text(f"❌ Error generating logo: `{e}`")


# ─────────────────────────────────────────────
#  .ylogo — black bg + yellow stroke
# ─────────────────────────────────────────────
@on_message("ylogo", allow_stan=True, Bad_user=True)
async def ylogo(client: Client, message: Message):
    text = message.text.split(' ', 1)[1] if ' ' in message.text else None
    if not text:
        return await message.reply_text("Please provide text after `.ylogo`")

    msg = await message.reply_text("⏳ Processing...")
    try:
        img = Image.open(BLACK_BG)
        font = _random_font(800)
        _draw_text(img, text, font, fill=(255, 255, 255), stroke_fill="yellow", stroke_width=25)
        img.save(OUTPUT, "png")
        await _send_logo(message, msg, OUTPUT)
    except Exception as e:
        await msg.delete()
...
# ─────────────────────────────────────────────
#  .rlogo — black bg + red stroke
# ─────────────────────────────────────────────
@on_message("rlogo", allow_stan=True, Bad_user=True)
async def rlogo(client: Client, message: Message):
    text = message.text.split(' ', 1)[1] if ' ' in message.text else None
    if not text:
        return await message.reply_text("Please provide text after `.rlogo`")

    msg = await message.reply_text("⏳ Processing...")
    try:
        img = Image.open(BLACK_BG)
        font = _random_font(800)
        _draw_text(img, text, font, fill=(255, 255, 255), stroke_fill="red", stroke_width=25)
        img.save(OUTPUT, "png")
        await _send_logo(message, msg, OUTPUT)
    except Exception as e:
        await msg.delete()
...
# ─────────────────────────────────────────────
#  .vlogo — black bg + dark magenta stroke
# ─────────────────────────────────────────────
@on_message("vlogo", allow_stan=True, Bad_user=True)
async def vlogo(client: Client, message: Message):
    text = message.text.split(' ', 1)[1] if ' ' in message.text else None
    if not text:
        return await message.reply_text("Please provide text after `.vlogo`")

    msg = await message.reply_text("⏳ Processing...")
    try:
        img = Image.open(BLACK_BG)
        font = _random_font(800)
        _draw_text(img, text, font, fill=(65, 105, 225), stroke_fill="DarkMagenta", stroke_width=25)
        img.save(OUTPUT, "png")
        await _send_logo(message, msg, OUTPUT)
    except Exception as e:
        await msg.delete()
...
# ─────────────────────────────────────────────
#  .blogo — black bg + blue stroke
# ─────────────────────────────────────────────
@on_message("blogo", allow_stan=True, Bad_user=True)
async def blogo(client: Client, message: Message):
    text = message.text.split(' ', 1)[1] if ' ' in message.text else None
    if not text:
        return await message.reply_text("Please provide text after `.blogo`")

    msg = await message.reply_text("⏳ Processing...")
    try:
        img = Image.open(BLACK_BG)
        font = _random_font(800)
        _draw_text(img, text, font, fill=(255, 255, 255), stroke_fill="blue", stroke_width=25)
        img.save(OUTPUT, "png")
        await _send_logo(message, msg, OUTPUT)
    except Exception as e:
        await msg.delete()
...
HelpMenu("logo").add(
    "logo",
    "<text>",
    "Generate a logo with a random background image and white text!",
    "logo PBX",
    "Background is randomly picked each time.",
).add(
    "ylogo",
    "<text>",
    "Generate a logo with black background and yellow stroke effect!",
    "ylogo PBX",
).add(
    "rlogo",
    "<text>",
    "Generate a logo with black background and red stroke effect!",
    "rlogo PBX",
).add(
    "vlogo",
    "<text>",
    "Generate a logo with black background and dark magenta/violet stroke effect!",
    "vlogo PBX",
).add(
    "blogo",
    "<text>",
    "Generate a logo with black background and blue stroke effect!",
    "blogo PBX",
).info(
    "Logo Maker 🎨"
).done()
